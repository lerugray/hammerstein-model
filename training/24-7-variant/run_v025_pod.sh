#!/bin/bash
# v0.2.5 continued-LoRA on RunPod (A5000 SECURE or 4090) — voice-fix
...
# Pipeline:
#   [1/5] Repo + deps
#   [2/5] Data validation (v0.2.5 additions + v0.1 Ray-stack + v3a synthetic)
...
SFT_OUTPUT=training/24-7-variant/output/qwen7b-v025-continued
GGUF_OUTPUT=training/24-7-variant/output/qwen7b-v025-q5km
RESULTS_DIR=training/24-7-variant/results
RUN_DATE=$(date -u +%Y-%m-%d)
COST_CEILING_USD=20

V025_ADDITIONS=data/ray-stack-sft-v0.2.5-additions.jsonl
...
echo "=== Qwen2.5-7B v0.2.5 continued-LoRA ==="
date
...
if [ ! -f /tmp/v025-deps-installed ]; then
...
for f in "$V025_ADDITIONS" "$V01_RAY_STACK" "$V3A_SYNTHETIC"; do
    if [ ! -f "$f" ]; then
        echo "ERROR: Required data file missing: $f"
        echo "  v0.2.5 additions: run scripts/v025_concat_sanitize.py locally + commit."
...
if [ ! -f "$SFT_OUTPUT/lora-adapter/adapter_config.json" ]; then
    echo ""
    echo "[3/5] Continued LoRA training (Qwen2.5-7B + v3a adapter + v0.2.5 mix)..."
    echo "      LR 2e-4, 2 epochs, eff batch 8, max_seq 2048"
    echo "      Expected: ~30 min on A5000 / RTX 4090"
    python training/24-7-variant/train_v025_continued.py \
        --v025-additions "$V025_ADDITIONS" \
...
if [ ! -f "$GGUF_OUTPUT/hammerstein-7b-v025-q5_k_m.gguf" ]; then
    echo ""
    ...
    GGUF_Q5="$GGUF_OUTPUT/hammerstein-7b-v025-q5_k_m.gguf"
...
HF_REPO_ID="${HF_REPO_ID:-lerugray/hammerstein-7b-v025}"
...
repo_id = "$HF_REPO_ID"
gguf_path = "$GGUF_OUTPUT/hammerstein-7b-v025-q5_k_m.gguf"
adapter_path = "$SFT_OUTPUT/lora-adapter-v025.tar.gz"
...
with open("/workspace/v025-hf-upload-meta.txt", "w") as f:
...
api.upload_file(
    ...
    path_in_repo="hammerstein-7b-v025-q5_k_m.gguf",
    repo_id=repo_id,
    repo_type="model",
    token=token,
)
...
api.upload_file(
    ...
    path_in_repo="lora-adapter-v025.tar.gz",
    repo_id=repo_id,
    repo_type="model",
    token=token,
)
...
with open("/workspace/v025-hf-upload-done", "w") as f:
    f.write(f"DONE\nrepo_id={repo_id}\ngguf_sha256={local_hash}\n")
print("Upload complete. Sentinel: /workspace/v025-hf-upload-done")
PYEOF

echo ""
echo "=== v0.2.5 training + HF upload complete ==="
date
echo ""
