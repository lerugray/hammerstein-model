#!/usr/bin/env python3
r"""v0.2.9 pod fire (apply_chat_template fix retrain).
...
LOCAL_STORE = Path("D:/hammerstein-store/models/v0.2.9")

RUNPOD_API = "https://api.runpod.io/graphql"

HF_REPO_ID = "lerugray/hammerstein-7b-v029"
...
def fire_pod(api_key: str, pubkey: str, gpu_type_id: str,
    ...
    variables = {
        "input": {
            ...
            "name": f"hammerstein-v029-{int(time.time())}",
...
def main() -> int:
    ...
    sys.stdout.flush(); print("=== v0.2.9 pod fire (apply_chat_template fix) ===")
    ...
    try:
        ...
        sys.stdout.flush(); print()
        sys.stdout.flush(); print("  Launching run_v029_pod.sh in tmux session 'v029'...")
        launch = (
            "cd /workspace/hammerstein-model && "
            "tmux new-session -d -s v029 "
            "'bash training/24-7-variant/run_v029_pod.sh > /workspace/v029-run.log 2>&1' && "
            "tmux ls"
        )
        ...
        sentinel_path = "/workspace/v029-hf-upload-done"
        ...
        while time.time() < deadline:
            ...
            try:
                check = (f"if [ -f {sentinel_path} ]; then echo READY; cat {sentinel_path}; "
                         f"else wc -c < /workspace/v029-run.log 2>/dev/null || echo 0; fi")
            ...
            if log_size != last_log_size:
                try:
                    rc2, tail = ssh_run(ip, port, "tail -8 /workspace/v029-run.log 2>/dev/null", timeout=30)
        ...
        if not upload_done:
            ...
            try:
                _, tail = ssh_run(ip, port, "tail -120 /workspace/v029-run.log 2>/dev/null", timeout=60)
        ...
        gguf_filename = "hammerstein-7b-v029-q5_k_m.gguf"
        adapter_filename = "lora-adapter-v029.tar.gz"
    ...
    sys.stdout.flush(); print()
    sys.stdout.flush(); print("=== v0.2.9 pod fire complete ===")
    sys.stdout.flush(); print(f"GGUF: {LOCAL_STORE}/hammerstein-7b-v029-q5_k_m.gguf")
    return 0
