#!/usr/bin/env python3
r"""v0.2.4 pod fire — local driver for autonomous RunPod training.
...
Lifecycle:
  ...
  5. SSH in, clone repo (or pull), plant HF_TOKEN at /workspace/.hf_token
     via SSH stdin, then launch run_v024_pod.sh in a detached tmux session
  6. Poll for /workspace/v024-hf-upload-done sentinel (pod pushes the
     GGUF + LoRA tar to a private HF repo as the transfer channel)
  7. hf_hub_download GGUF + adapter to D:\hammerstein-store\models\v0.2.4\
...
LOCAL_STORE = Path("D:/hammerstein-store/models/v0.2.4")
...
HF_REPO_ID = "lerugray/hammerstein-7b-v024"
...
def ensure_ssh_keypair() -> str:
    ...
    if not KEY_PATH.exists():
        ...
        subprocess.run(
            ["ssh-keygen", "-t", "ed25519", "-N", "", "-f", str(KEY_PATH), "-C", "v024-pod"],
            check=True,
        )
...
def fire_pod(api_key: str, pubkey: str, gpu_type_id: str,
    ...
    variables = {
        "input": {
            ...
            "name": f"hammerstein-v024-{int(time.time())}",
            "env": [
...
def scp_back(ip: str, port: int, remote_path: str, local_path: Path) -> None:
    ...
    # 60 min — the v0.2.4 fire on 2026-05-23 transferred a ~5GB GGUF in ~28
...
def main() -> int:
    ...
    sys.stdout.flush(); print("=== v0.2.4 pod fire ===")
    ...
    try:
        ...
        sys.stdout.flush(); print()
        sys.stdout.flush(); print("  Launching run_v024_pod.sh in tmux session 'v024'...")
        launch = (
            "cd /workspace/hammerstein-model && "
            "tmux new-session -d -s v024 "
            "'bash training/24-7-variant/run_v024_pod.sh > /workspace/v024-run.log 2>&1' && "
            "tmux ls"
        )
        ...
        # /workspace/v024-hf-upload-done when both are uploaded. This is
        ...
        sentinel_path = "/workspace/v024-hf-upload-done"
        ...
        while time.time() < deadline:
            ...
            try:
                check = (
                    ...
                    f"else wc -c < /workspace/v024-run.log 2>/dev/null || echo 0; fi"
                )
            ...
            if log_size != last_log_size:
                try:
                    rc2, tail = ssh_run(ip, port, "tail -8 /workspace/v024-run.log 2>/dev/null", timeout=30)
        ...
        if not upload_done:
            ...
            try:
                _, tail = ssh_run(ip, port, "tail -80 /workspace/v024-run.log 2>/dev/null", timeout=60)
        ...
        gguf_filename = "hammerstein-7b-v024-q5_k_m.gguf"
        adapter_filename = "lora-adapter-v024.tar.gz"
    ...
    sys.stdout.flush(); print()
    sys.stdout.flush(); print("=== v0.2.4 pod fire complete ===")
    sys.stdout.flush(); print()
    sys.stdout.flush(); print(f"GGUF: {LOCAL_STORE}/hammerstein-7b-v024-q5_k_m.gguf")
    sys.stdout.flush(); print()
    sys.stdout.flush(); print("Next steps (local):")
    sys.stdout.flush(); print(f"  1. Build Modelfile.v024 (copy Modelfile.v023, point at the new GGUF)")
    sys.stdout.flush(); print(f"  2. ollama create hammerstein-7b-v024 -f deploy/Modelfile.v024")
    sys.stdout.flush(); print(f"  3. python scripts/v023_self_state_probe.py --model hammerstein-7b-v024 --tag v024-post-train")
    sys.stdout.flush(); print(f"  4. python scripts/v023_voice_probe.py --model hammerstein-7b-v024 --tag v024-post-train")
    sys.stdout.flush(); print(f"  5. Tool-call smoke: python scripts/rung1_chat.py via sidecar pointed at v024")
    sys.stdout.flush(); print(f"  6. Compare against v023 evals (data/eval-*-v023-*.json). If v024 matches/beats: cut homelab over.")
    return 0
