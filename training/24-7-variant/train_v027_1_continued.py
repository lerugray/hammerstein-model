#!/usr/bin/env python3
"""v0.2.7.1 continued-LoRA training — iteration on v0.2.7.

Same recipe as train_v027_continued.py but:
  - Base adapter is v0.2.7 (HF repo lerugray/hammerstein-7b-v027,
    file lora-adapter-v027.tar.gz)
  - Additions file is data/ray-stack-sft-v0.2.7.1-additions.jsonl
    (833 examples — v0.2.7's 603 + 230 fix-data oversampled at 10x)

Training mix (target ~3,290 examples):
- 833 v0.2.7.1 additions x 3 baseline = 2,499
- 541 v0.1 ray-stack x 1               = 541
- 250 v3a synthetic sample             = 250
- TOTAL                                = 3,290

Estimated train time: ~12-15 min on H100/H200; ~30 min on A6000.

Revert point: v0.2.6.2 (the validated regression floor).
"""
...
# v0.2.7.1 STARTS from the v0.2.7 adapter. Pod-side bootstrap downloads
# the tar from HF and extracts to BASE_ADAPTER_PATH before training.
BASE_ADAPTER_PATH_DEFAULT = "/workspace/v027-adapter"
V027_HF_REPO = "lerugray/hammerstein-7b-v027"
V027_ADAPTER_TAR = "lora-adapter-v027.tar.gz"
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--v027-1-additions",
                   default="data/ray-stack-sft-v0.2.7.1-additions.jsonl")
    ...
    p.add_argument("--base-adapter", default=BASE_ADAPTER_PATH_DEFAULT,
                   help="Path to the v0.2.7 adapter dir (pod-side; pre-extracted from HF tar).")
    p.add_argument("--output",
                   default="training/24-7-variant/output/qwen7b-v027-1-continued")
...
def build_mix(args: argparse.Namespace) -> tuple[list[str], dict]:
    ...
    v_path = Path(args.v027_1_additions)
    if v_path.exists():
        v_rows = load_jsonl(args.v027_1_additions)
        for _ in range(MIX_TARGETS["v027_1_additions_oversample"]):
            for row in v_rows:
                text = format_example(row)
                if text:
                    examples.append(text)
        counts["v027_1_additions"] = len(v_rows) * MIX_TARGETS["v027_1_additions_oversample"]
    else:
        print(f"  ERROR: v0.2.7.1 additions missing: {args.v027_1_additions}")
        counts["v027_1_additions"] = 0
...
def run_training(args: argparse.Namespace) -> None:
    ...
    print(f"\nLoading v0.2.7 base adapter from {args.base_adapter} (trainable=True)...")
    if not Path(args.base_adapter).exists():
        sys.exit(f"ERROR: base adapter dir not found at {args.base_adapter}. "
                 f"Pod bootstrap should extract HF tar {V027_HF_REPO}/{V027_ADAPTER_TAR} here.")
    ...
    print("\nStarting continued training (v0.2.7.1)...")
...
def main() -> None:
    args = parse_args()
    print("=== Qwen2.5-7B v0.2.7.1 continued-LoRA SFT ===")
    print(f"  Base model:          {BASE_MODEL}")
    print(f"  Base adapter:        {args.base_adapter}  (v0.2.7 continued)")
    print(f"  v0.2.7.1 additions:  {args.v027_1_additions} (x{MIX_TARGETS['v027_1_additions_oversample']} baseline oversample)")
    ...
    if not Path(args.v027_1_additions).exists():
        print(f"ERROR: v0.2.7.1 additions not found at {args.v027_1_additions}")
        print(f"  Run: python scripts/v027_1_concat_sanitize.py")
        sys.exit(1)
