#!/bin/bash
# v0.2.7 continued-LoRA on RunPod — iteration on v0.2.6.2 with 11-axis
# data design and continued-from-v0.2.6.2 starting adapter.
set -e

REPO_DIR=/workspace/hammerstein-model
SFT_OUTPUT=training/24-7-variant/output/qwen7b-v027-continued
GGUF_OUTPUT=training/24-7-variant/output/qwen7b-v027-q5km
RESULTS_DIR=training/24-7-variant/results
COST_CEILING_USD=30

V027_ADDITIONS=data/ray-stack-sft-v0.2.7-additions.jsonl
V01_RAY_STACK=data/ray-stack-sft-v0.1-combined.jsonl
V3A_SYNTHETIC=tools/distill/data/synthetic-v3a-2026-05-09.jsonl

# v0.2.7 continues from the v0.2.6.2 adapter. Pulled from the private HF
# repo that v0.2.6.2's pod runner pushed to.
...
echo "=== Qwen2.5-7B v0.2.7 continued-LoRA ==="
date
...
cd "$REPO_DIR"
# v0.2.7 files are on master (Mac orchestrator committed there directly).
git fetch --all && git checkout master && git pull origin master

if [ ! -f /tmp/v027-deps-installed ]; then
...
for f in "$V027_ADDITIONS" "$V01_RAY_STACK" "$V3A_SYNTHETIC"; do
...
# Tar's top-level dir is "lora-adapter" (per run_v026_2_pod.sh)
src = staging / "lora-adapter"
if not src.exists():
    # Fallback: maybe single-dir structure differs; pick the first dir
...
if [ ! -f "$SFT_OUTPUT/lora-adapter/adapter_config.json" ]; then
    echo ""
    echo "[4/6] Continued LoRA training (v0.2.7 from v0.2.6.2 adapter)..."
    python training/24-7-variant/train_v027_continued.py \
        --v027-additions "$V027_ADDITIONS" \
...
if [ ! -f "$GGUF_OUTPUT/hammerstein-7b-v027-q5_k_m.gguf" ]; then
    echo ""
    ...
    GGUF_Q5="$GGUF_OUTPUT/hammerstein-7b-v027-q5_k_m.gguf"
...
HF_REPO_ID="${HF_REPO_ID:-lerugray/hammerstein-7b-v027}"

if [ -z "$HF_TOKEN" ]; then echo "ERROR: HF_TOKEN not set"; exit 1; fi
if [ ! -f "$GGUF_OUTPUT/hammerstein-7b-v027-q5_k_m.gguf" ]; then echo "ERROR: GGUF missing"; exit 1; fi
...
token = os.environ["HF_TOKEN"]
repo_id = "$HF_REPO_ID"
gguf_path = "$GGUF_OUTPUT/hammerstein-7b-v027-q5_k_m.gguf"
adapter_path = "$SFT_OUTPUT/lora-adapter-v027.tar.gz"
...
with open("/workspace/v027-hf-upload-meta.txt", "w") as f:
    f.write(f"repo_id={repo_id}\n")
    f.write(f"gguf_sha256={local_hash}\n")

print(f"Uploading GGUF ({os.path.getsize(gguf_path)/1e9:.2f} GB)...")
api.upload_file(path_or_fileobj=gguf_path, path_in_repo="hammerstein-7b-v027-q5_k_m.gguf",
                repo_id=repo_id, repo_type="model", token=token)
print("GGUF uploaded.")

print(f"Uploading LoRA adapter tar...")
api.upload_file(path_or_fileobj=adapter_path, path_in_repo="lora-adapter-v027.tar.gz",
                repo_id=repo_id, repo_type="model", token=token)
print("Adapter uploaded.")

with open("/workspace/v027-hf-upload-done", "w") as f:
    f.write(f"DONE\nrepo_id={repo_id}\ngguf_sha256={local_hash}\n")
print("Sentinel: /workspace/v027-hf-upload-done")
PYEOF

echo ""
echo "=== v0.2.7 training + HF upload complete ==="
date
