#!/bin/bash
# v0.2.7.1 continued-LoRA on RunPod — iteration on v0.2.7 with self-state
# fabrication fix + low-signal extraction edge cases + anti-meta-leakage-v3.
set -e

REPO_DIR=/workspace/hammerstein-model
SFT_OUTPUT=training/24-7-variant/output/qwen7b-v027-1-continued
GGUF_OUTPUT=training/24-7-variant/output/qwen7b-v027-1-q5km
RESULTS_DIR=training/24-7-variant/results
COST_CEILING_USD=30

V027_1_ADDITIONS=data/ray-stack-sft-v0.2.7.1-additions.jsonl
V01_RAY_STACK=data/ray-stack-sft-v0.1-combined.jsonl
V3A_SYNTHETIC=tools/distill/data/synthetic-v3a-2026-05-09.jsonl

# v0.2.7.1 continues from the v0.2.7 adapter (NOT v0.2.6.2 — v0.2.7's
# voice gains are worth preserving; the fix data tightens specific
# failure modes without re-deriving the broader behavior).
V027_HF_REPO=lerugray/hammerstein-7b-v027
V027_ADAPTER_TAR=lora-adapter-v027.tar.gz
BASE_ADAPTER_DIR=/workspace/v027-adapter

cd /workspace 2>/dev/null || cd ~

echo "=== Qwen2.5-7B v0.2.7.1 continued-LoRA ==="
date
...
if [ ! -f /tmp/v027-1-deps-installed ]; then
    ...
    touch /tmp/v027-1-deps-installed
fi
...
for f in "$V027_1_ADDITIONS" "$V01_RAY_STACK" "$V3A_SYNTHETIC"; do
    if [ ! -f "$f" ]; then echo "ERROR: missing $f"; exit 1; fi
    echo "  $f  ($(wc -l < "$f") lines)"
done

echo ""
echo "[3/6] Pulling v0.2.7 base adapter from HF..."
...
staging = Path("/workspace/v027-adapter-staging")
...
print(f"  Files: {sorted(p.name for p in target.iterdir())}")
PYEOF
else
...
if [ ! -f "$SFT_OUTPUT/lora-adapter/adapter_config.json" ]; then
    echo ""
    echo "[4/6] Continued LoRA training (v0.2.7.1 from v0.2.7 adapter)..."
    python training/24-7-variant/train_v027_1_continued.py \
        --v027-1-additions "$V027_1_ADDITIONS" \
...
if [ ! -f "$GGUF_OUTPUT/hammerstein-7b-v027-1-q5_k_m.gguf" ]; then
    echo ""
    ...
    GGUF_Q5="$GGUF_OUTPUT/hammerstein-7b-v027-1-q5_k_m.gguf"
...
HF_REPO_ID="${HF_REPO_ID:-lerugray/hammerstein-7b-v027-1}"

if [ -z "$HF_TOKEN" ]; then echo "ERROR: HF_TOKEN not set"; exit 1; fi
if [ ! -f "$GGUF_OUTPUT/hammerstein-7b-v027-1-q5_k_m.gguf" ]; then echo "ERROR: GGUF missing"; exit 1; fi
...
token = os.environ["HF_TOKEN"]
repo_id = "$HF_REPO_ID"
gguf_path = "$GGUF_OUTPUT/hammerstein-7b-v027-1-q5_k_m.gguf"
adapter_path = "$SFT_OUTPUT/lora-adapter-v027-1.tar.gz"
...
with open("/workspace/v027-1-hf-upload-meta.txt", "w") as f:
    f.write(f"repo_id={repo_id}\n")
    f.write(f"gguf_sha256={local_hash}\n")

print(f"Uploading GGUF ({os.path.getsize(gguf_path)/1e9:.2f} GB)...")
api.upload_file(path_or_fileobj=gguf_path, path_in_repo="hammerstein-7b-v027-1-q5_k_m.gguf",
                repo_id=repo_id, repo_type="model", token=token)
print("GGUF uploaded.")

print(f"Uploading LoRA adapter tar...")
api.upload_file(path_or_fileobj=adapter_path, path_in_repo="lora-adapter-v027-1.tar.gz",
                repo_id=repo_id, repo_type="model", token=token)
print("Adapter uploaded.")

with open("/workspace/v027-1-hf-upload-done", "w") as f:
    f.write(f"DONE\nrepo_id={repo_id}\ngguf_sha256={local_hash}\n")
print("Sentinel: /workspace/v027-1-hf-upload-done")
PYEOF

echo ""
echo "=== v0.2.7.1 training + HF upload complete ==="
date
