#!/usr/bin/env python3
"""v0.2.8 clean-rebase LoRA training — from v0.2.6.2 base, single pass.

Per orchestrator handoff
(docs/handoffs/v0.2.8-rebase-from-privategs-orchestrator-2026-05-25.md):
clean-rebase from v0.2.6.2 base adapter with ALL accumulated v0.2.7.x
additions in a single training pass. This is hm-013, promoted from
"before v0.2.8" to "instead of v0.2.7.3" after v0.2.7.2's catastrophic
regression demonstrated drift in the 5-deep continued-LoRA stack.

Training mix (target ~2,500-2,700 examples):
- 620 v0.2.8 additions x 3 baseline = 1,860
- 541 v0.1 ray-stack x 1            = 541
- 250 v3a synthetic sample          = 250
- TOTAL                             = 2,651

Base adapter: v0.2.6.2 (NOT v0.2.7.1 or v0.2.7.2). This is the clean reset.
"""
...
# v0.2.8 clean-rebase: continues from v0.2.6.2 (NOT from v0.2.7.x stack).
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--v028-additions",
                   default="data/ray-stack-sft-v0.2.8-additions.jsonl")
    ...
    p.add_argument("--output",
                   default="training/24-7-variant/output/qwen7b-v028-continued")
...
def build_mix(args: argparse.Namespace) -> tuple[list[str], dict]:
    ...
    v_path = Path(args.v028_additions)
    if v_path.exists():
        v_rows = load_jsonl(args.v028_additions)
        for _ in range(MIX_TARGETS["v028_additions_oversample"]):
            for row in v_rows:
                text = format_example(row)
                if text:
                    examples.append(text)
        counts["v028_additions"] = len(v_rows) * MIX_TARGETS["v028_additions_oversample"]
    else:
        print(f"  ERROR: v0.2.8 additions missing: {args.v028_additions}")
        counts["v028_additions"] = 0
...
def run_training(args: argparse.Namespace) -> None:
    ...
    print("\nStarting clean-rebase training (v0.2.8 from v0.2.6.2 adapter)...")
    ...
    print(f"\nClean-rebase adapter saved to {adapter_dir}")
...
def main() -> None:
    args = parse_args()
    print("=== Qwen2.5-7B v0.2.8 clean-rebase SFT ===")
    print(f"  Base model:        {BASE_MODEL}")
    print(f"  Base adapter:      {args.base_adapter}  (v0.2.6.2 — clean rebase)")
    print(f"  v0.2.8 additions:  {args.v028_additions} (x{MIX_TARGETS['v028_additions_oversample']} baseline)")
    ...
    if not Path(args.v028_additions).exists():
        print(f"ERROR: v0.2.8 additions not found at {args.v028_additions}")
        sys.exit(1)
