#!/bin/bash
# v0.2.9.2 clean-rebase LoRA on RunPod with apply_chat_template fix.
set -e

REPO_DIR=/workspace/hammerstein-model
SFT_OUTPUT=training/24-7-variant/output/qwen7b-v029-2-continued
GGUF_OUTPUT=training/24-7-variant/output/qwen7b-v029-2-q5km
RESULTS_DIR=training/24-7-variant/results
COST_CEILING_USD=30

V029_ADDITIONS=data/ray-stack-sft-v0.2.9.2-additions.jsonl
...
BASE_ADAPTER_DIR=/workspace/v029-1-adapter

cd /workspace 2>/dev/null || cd ~

echo "=== Qwen2.5-7B v0.2.9.2 clean-rebase LoRA (with apply_chat_template) ==="
date
...
if [ ! -f /tmp/v029-2-deps-installed ]; then
    ...
    touch /tmp/v029-2-deps-installed
fi
...
echo ""
echo "[3/6] Pulling v0.2.9.1 base adapter from HF..."
...
staging = Path("/workspace/v029-1-adapter-staging")
...
if [ ! -f "$SFT_OUTPUT/lora-adapter/adapter_config.json" ]; then
    echo ""
    echo "[4/6] v0.2.9.2 LoRA training with apply_chat_template..."
    python training/24-7-variant/train_v029_2_continued.py \
        --v029-2-additions "$V029_ADDITIONS" \
...
if [ ! -f "$GGUF_OUTPUT/hammerstein-7b-v029-2-q5_k_m.gguf" ]; then
    echo ""
    ...
    GGUF_Q5="$GGUF_OUTPUT/hammerstein-7b-v029-2-q5_k_m.gguf"
...
HF_REPO_ID="${HF_REPO_ID:-lerugray/hammerstein-7b-v029-2}"

if [ -z "$HF_TOKEN" ]; then echo "ERROR: HF_TOKEN not set"; exit 1; fi
if [ ! -f "$GGUF_OUTPUT/hammerstein-7b-v029-2-q5_k_m.gguf" ]; then echo "ERROR: GGUF missing"; exit 1; fi
...
token = os.environ["HF_TOKEN"]
repo_id = "$HF_REPO_ID"
gguf_path = "$GGUF_OUTPUT/hammerstein-7b-v029-2-q5_k_m.gguf"
adapter_path = "$SFT_OUTPUT/lora-adapter-v029-2.tar.gz"
...
with open("/workspace/v029-2-hf-upload-meta.txt", "w") as f:
    f.write(f"repo_id={repo_id}\n")
    f.write(f"gguf_sha256={local_hash}\n")

print(f"Uploading GGUF ({os.path.getsize(gguf_path)/1e9:.2f} GB)...")
api.upload_file(path_or_fileobj=gguf_path, path_in_repo="hammerstein-7b-v029-2-q5_k_m.gguf",
                repo_id=repo_id, repo_type="model", token=token)
print("GGUF uploaded.")

print(f"Uploading LoRA adapter tar...")
api.upload_file(path_or_fileobj=adapter_path, path_in_repo="lora-adapter-v029-2.tar.gz",
                repo_id=repo_id, repo_type="model", token=token)
print("Adapter uploaded.")

with open("/workspace/v029-2-hf-upload-done", "w") as f:
    f.write(f"DONE\nrepo_id={repo_id}\ngguf_sha256={local_hash}\n")
print("Sentinel: /workspace/v029-2-hf-upload-done")
PYEOF

echo ""
echo "=== v0.2.9.2 training + HF upload complete ==="
date
