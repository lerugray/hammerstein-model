#!/usr/bin/env python3
"""v0.2.9.2 continued-LoRA training with tokenizer.apply_chat_template fix.

Per docs/handoffs/v0.2.9.2-from-privategs-orchestrator-2026-05-25.md:
...
v0.2.9.2 tool-use: 30% → 70-90%.

Training mix (target ~2,700 examples):
- 660 v0.2.9.2 additions x 3 baseline = 1,980
- 541 v0.1 ray-stack x 1            = 541
- 250 v3a synthetic sample          = 250
- TOTAL                             = 2,771

Base adapter: v0.2.9.1 (CONTINUED LoRA, 2-level stack from v0.2.6.2
clean-rebase → v0.2.9.1 → v0.2.9.2). Per training-prep-discipline memory,
3-level depth is comfort zone; v0.2.9.2 sits at 2.
"""
...
BASE_ADAPTER_PATH_DEFAULT = "/workspace/v029-1-adapter"
V029_1_HF_REPO = "lerugray/hammerstein-7b-v029-1"
V029_1_ADAPTER_TAR = "lora-adapter-v029-1.tar.gz"
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--v029-2-additions",
                   default="data/ray-stack-sft-v0.2.9.2-additions.jsonl")
    ...
    p.add_argument("--output",
                   default="training/24-7-variant/output/qwen7b-v029-2-continued")
...
def format_example(row: dict, tokenizer) -> str:
    """v0.2.9.2 format_example — uses tokenizer.apply_chat_template for
...
def build_mix(args: argparse.Namespace, tokenizer=None) -> tuple[list[str], dict]:
    ...
    v_path = Path(args.v029_2_additions)
    if v_path.exists():
        v_rows = load_jsonl(args.v029_2_additions)
        for _ in range(MIX_TARGETS["v029_2_additions_oversample"]):
            for row in v_rows:
                text = fmt(row)
                if text:
                    examples.append(text)
        counts["v029_2_additions"] = len(v_rows) * MIX_TARGETS["v029_2_additions_oversample"]
    else:
        print(f"  ERROR: v0.2.9.2 additions missing: {args.v029_2_additions}")
        counts["v029_2_additions"] = 0
...
def run_training(args: argparse.Namespace) -> None:
    ...
    print(f"\nLoading v0.2.9.1 base adapter from {args.base_adapter} (trainable=True)...")
    if not Path(args.base_adapter).exists():
        sys.exit(f"ERROR: base adapter dir not found at {args.base_adapter}. "
                 f"Pod bootstrap should extract HF tar {V029_1_HF_REPO}/{V029_1_ADAPTER_TAR} here.")
    ...
    print("\nStarting v0.2.9.2 continued-LoRA training with apply_chat_template fix...")
    ...
    print(f"\nv0.2.9.2 adapter saved to {adapter_dir}")
...
def main() -> None:
    args = parse_args()
    print("=== Qwen2.5-7B v0.2.9.2 continued-LoRA with apply_chat_template fix ===")
    print(f"  Base model:        {BASE_MODEL}")
    print(f"  Base adapter:      {args.base_adapter}  (v0.2.9.1 — continued LoRA)")
    print(f"  v0.2.9.2 additions:  {args.v029_2_additions} (x{MIX_TARGETS['v029_2_additions_oversample']} baseline)")
    ...
    if not Path(args.v029_2_additions).exists():
        print(f"ERROR: v0.2.9.2 additions not found at {args.v029_2_additions}")
        sys.exit(1)
