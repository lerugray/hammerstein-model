#!/usr/bin/env python3
"""v0.2.9.1 clean-rebase LoRA training with tokenizer.apply_chat_template fix.

Per docs/handoffs/v0.2.9.1-from-privategs-orchestrator-2026-05-25.md:
...
v0.2.9.1 tool-use: 30% → 70-90%.

Training mix (target ~2,700 examples):
- 660 v0.2.9.1 additions x 3 baseline = 1,980
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--v029-1-additions",
                   default="data/ray-stack-sft-v0.2.9.1-additions.jsonl")
    ...
    p.add_argument("--output",
                   default="training/24-7-variant/output/qwen7b-v029-1-continued")
...
def format_example(row: dict, tokenizer) -> str:
    """v0.2.9.1 format_example — uses tokenizer.apply_chat_template for
...
def build_mix(args: argparse.Namespace, tokenizer=None) -> tuple[list[str], dict]:
    ...
    v_path = Path(args.v029_1_additions)
    if v_path.exists():
        v_rows = load_jsonl(args.v029_1_additions)
        for _ in range(MIX_TARGETS["v029_1_additions_oversample"]):
            for row in v_rows:
                text = fmt(row)
                if text:
                    examples.append(text)
        counts["v029_1_additions"] = len(v_rows) * MIX_TARGETS["v029_1_additions_oversample"]
    else:
        print(f"  ERROR: v0.2.9.1 additions missing: {args.v029_1_additions}")
        counts["v029_1_additions"] = 0
...
def run_training(args: argparse.Namespace) -> None:
    ...
    print("\nStarting v0.2.9.1 clean-rebase training with apply_chat_template fix...")
    ...
    print(f"\nv0.2.9.1 adapter saved to {adapter_dir}")
...
def main() -> None:
    args = parse_args()
    print("=== Qwen2.5-7B v0.2.9.1 clean-rebase with apply_chat_template fix ===")
    print(f"  Base model:        {BASE_MODEL}")
    print(f"  Base adapter:      {args.base_adapter}  (v0.2.6.2 — clean rebase)")
    print(f"  v0.2.9.1 additions:  {args.v029_1_additions} (x{MIX_TARGETS['v029_1_additions_oversample']} baseline)")
    ...
    if not Path(args.v029_1_additions).exists():
        print(f"ERROR: v0.2.9.1 additions not found at {args.v029_1_additions}")
        sys.exit(1)
