#!/usr/bin/env python3
"""v0.2.9.1 additions concat + sanitize.

Per docs/handoffs/v0.2.9.1-from-privategs-orchestrator-2026-05-25.md:
clean-rebase from v0.2.6.2 base (same as v0.2.8/v0.2.9) with the
v0.2.9 inclusion list PLUS two new subagent-authored files:

  - data/v0.2.9.1-tool-use-judgment-expanded.jsonl (83 pairs)
  - data/v0.2.9.1-high-signal-extraction-reinforcement.jsonl (47 rows)

Both new files at 5x oversample.

Expected ratio balance:
  - Tool-use: 93 pairs × 5x = 465 examples (~10x more than v0.2.9)
  - Extraction [] emission: ~105 examples
  - Extraction non-empty: ~270 examples
  - Ratio: 1:2.6 toward non-empty (reverses v0.2.7.2's 11:1 toward [])
"""
...
# (filename, oversample_factor) — orchestrator's v0.2.9.1 inclusion list
ADDITIONS_FILES: list[tuple[str, int]] = [
    ...
    ("v0.2.9-tool-routing-alignment-augmented.jsonl",       5),  # augmented w/ tools
    ("v0.2.9-tool-use-judgment-augmented.jsonl",            5),  # augmented w/ tools
    ...
    # v0.2.9.1 NEW — subagent-authored scale-up
    ("v0.2.9.1-tool-use-judgment-expanded.jsonl",           5),  # 83 pairs
    ("v0.2.9.1-high-signal-extraction-reinforcement.jsonl", 5),  # 47 rows
]

OUT = DATA_DIR / "ray-stack-sft-v0.2.9.1-additions.jsonl"
...
def main() -> int:
    ...
    print(f"v0.2.9.1 additions assembled at {OUT}")
    ...
    # Sanity: ratio of [] emission to non-empty extraction outputs
    empty_count = 0
    nonempty_count = 0
    tools_in_output = 0
