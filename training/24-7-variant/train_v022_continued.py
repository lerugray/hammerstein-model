#!/usr/bin/env python3
"""v0.2.2 continued-LoRA training — focused iteration after v0.2.1's
worse-than-v0.2 verdict.

Changes vs train_v021_continued.py:
- Re-add v3a synthetic at 250-pair sample (was 0 in v0.2.1, 500 in v0.2).
  The English-voice anchor the v0.2.1 retrain stripped; saved as the
  load-bearing finding in `project_v3a_synthetic_is_english_anchor.md`.
- Epochs 3 -> 2 (v0.2.1 verdict said 3 was overshoot).
- Add 41 new hand-written tool-call training pairs targeting our exact
  tools (library_search, library_read, web_search) plus no-tool casual
  examples. Targets the Rung 1 tool-call format-discipline gap that
  v3a's no-system-prompt design caused.

Kept from v0.2.1 (proven good or neutral):
- LR 2e-4
- Oversample 3x on the v0.2.2 additions
- The 20 short-prompt + 40 fabrication-discrimination pairs
- All the v0.2 voice anchor seeds + audit-trigger discrimination +
  OpenRouter restyled pairs

Training mix:
- data/ray-stack-sft-v0.2.2-additions.jsonl (354 pairs, oversampled 3x
  = 1,062)
- data/ray-stack-sft-v0.1-combined.jsonl (541 pairs, full)
- tools/distill/data/synthetic-v3a-2026-05-09.jsonl (sample 250)

Total mix ~1,853 pairs. Estimated A5000 ~$0.50 / ~45 min train.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--v022-additions",
                   default="data/ray-stack-sft-v0.2.2-additions.jsonl")
    ...
    p.add_argument("--output",
                   default="training/24-7-variant/output/qwen7b-v022-continued")
...
def build_mix(args: argparse.Namespace) -> tuple[list[str], dict]:
    ...
    v022_path = Path(args.v022_additions)
    if v022_path.exists():
        v022_rows = load_jsonl(args.v022_additions)
        for _ in range(MIX_TARGETS["v022_additions_oversample"]):
            for row in v022_rows:
                text = format_example(row)
                if text:
                    examples.append(text)
        counts["v022_additions"] = len(v022_rows) * MIX_TARGETS["v022_additions_oversample"]
    else:
        print(f"  WARN: v0.2.2 additions missing: {args.v022_additions}")
        counts["v022_additions"] = 0
...
def run_training(args: argparse.Namespace) -> None:
    ...
    print("\nStarting continued training (v0.2.2)...")
...
def main() -> None:
    args = parse_args()
    print("=== Qwen2.5-7B v0.2.2 continued-LoRA SFT ===")
    print(f"  Base model:       {BASE_MODEL}")
    print(f"  Starting adapter: {args.v3a_adapter}")
    print(f"  v0.2.2 additions: {args.v022_additions} (x{MIX_TARGETS['v022_additions_oversample']} oversample)")
    ...
    if not Path(args.v022_additions).exists():
        print(f"ERROR: v0.2.2 additions not found at {args.v022_additions}")
        sys.exit(1)
