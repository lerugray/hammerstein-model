#!/bin/bash
# v0.2.8 clean-rebase LoRA on RunPod — single pass from v0.2.6.2 base.
set -e

REPO_DIR=/workspace/hammerstein-model
SFT_OUTPUT=training/24-7-variant/output/qwen7b-v028-continued
GGUF_OUTPUT=training/24-7-variant/output/qwen7b-v028-q5km
RESULTS_DIR=training/24-7-variant/results
COST_CEILING_USD=30

V028_ADDITIONS=data/ray-stack-sft-v0.2.8-additions.jsonl
V01_RAY_STACK=data/ray-stack-sft-v0.1-combined.jsonl
V3A_SYNTHETIC=tools/distill/data/synthetic-v3a-2026-05-09.jsonl

# v0.2.8 clean-rebase starts from v0.2.6.2 adapter (skipping v0.2.7.x stack)
...
echo "=== Qwen2.5-7B v0.2.8 clean-rebase LoRA ==="
date
...
if [ ! -f /tmp/v028-deps-installed ]; then
...
for f in "$V028_ADDITIONS" "$V01_RAY_STACK" "$V3A_SYNTHETIC"; do
...
if [ ! -f "$SFT_OUTPUT/lora-adapter/adapter_config.json" ]; then
    echo ""
    echo "[4/6] Clean-rebase LoRA training (v0.2.8 from v0.2.6.2 adapter)..."
    python training/24-7-variant/train_v028_continued.py \
        --v028-additions "$V028_ADDITIONS" \
...
if [ ! -f "$GGUF_OUTPUT/hammerstein-7b-v028-q5_k_m.gguf" ]; then
    echo ""
    ...
    GGUF_Q5="$GGUF_OUTPUT/hammerstein-7b-v028-q5_k_m.gguf"
...
HF_REPO_ID="${HF_REPO_ID:-lerugray/hammerstein-7b-v028}"

if [ -z "$HF_TOKEN" ]; then echo "ERROR: HF_TOKEN not set"; exit 1; fi
if [ ! -f "$GGUF_OUTPUT/hammerstein-7b-v028-q5_k_m.gguf" ]; then echo "ERROR: GGUF missing"; exit 1; fi
...
token = os.environ["HF_TOKEN"]
repo_id = "$HF_REPO_ID"
gguf_path = "$GGUF_OUTPUT/hammerstein-7b-v028-q5_k_m.gguf"
adapter_path = "$SFT_OUTPUT/lora-adapter-v028.tar.gz"
...
with open("/workspace/v028-hf-upload-meta.txt", "w") as f:
    f.write(f"repo_id={repo_id}\n")
    f.write(f"gguf_sha256={local_hash}\n")

print(f"Uploading GGUF ({os.path.getsize(gguf_path)/1e9:.2f} GB)...")
api.upload_file(path_or_fileobj=gguf_path, path_in_repo="hammerstein-7b-v028-q5_k_m.gguf",
                repo_id=repo_id, repo_type="model", token=token)
print("GGUF uploaded.")

print(f"Uploading LoRA adapter tar...")
api.upload_file(path_or_fileobj=adapter_path, path_in_repo="lora-adapter-v028.tar.gz",
                repo_id=repo_id, repo_type="model", token=token)
print("Adapter uploaded.")

with open("/workspace/v028-hf-upload-done", "w") as f:
    f.write(f"DONE\nrepo_id={repo_id}\ngguf_sha256={local_hash}\n")
print("Sentinel: /workspace/v028-hf-upload-done")
PYEOF

echo ""
echo "=== v0.2.8 clean-rebase training + HF upload complete ==="
date
