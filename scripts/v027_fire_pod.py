#!/usr/bin/env python3
r"""v0.2.7 pod fire — H100/H200-first per Ray's "burn the runpod account"
override (2026-05-25). Falls back through A100 80GB → A6000 → 4090 if the
top tier is unavailable.

Mirror of v026_2_fire_pod.py with:
  - GPU preference flipped from A6000-first to H100/H200-first
  - Branch checkout to master (not v0.2.6-retrain) — v0.2.7 files live on master
  - HF repo bump to lerugray/hammerstein-7b-v027
  - Sentinel + log path bumps
"""
...
LOCAL_STORE = Path("D:/hammerstein-store/models/v0.2.7")

RUNPOD_API = "https://api.runpod.io/graphql"

HF_REPO_ID = "lerugray/hammerstein-7b-v027"
...
# Power-first per Ray override 2026-05-25: "use the absolute best pod for
# the job, don't care what it costs". H100/H200 are the fastest available
# at RunPod; A100 80GB is a strong backup; A6000/4090 are the handoff
# brief's original defaults (kept as fallback).
PREFERRED_GPU_TYPES = [
    # Top tier — Ray-override target
    "H200 SXM",                  # 141GB HBM3e — fastest available
    "H100 NVL",                  # 94GB HBM3
    "H100 SXM",                  # 80GB HBM3
    "H100 80GB HBM3",
    "H100 PCIe",
    ...
    "L40S",                      # 48GB
    "L40",                       # 48GB
    "RTX A6000",                 # 48GB — handoff default
    "RTX 4090",                  # 24GB — handoff fallback
    # Last resort
    "A40",                       # 48GB
    "RTX A5000",                 # 24GB
    "RTX A4500",                 # 20GB
...
def ensure_ssh_keypair() -> str:
    ...
    if not KEY_PATH.exists():
        ...
        subprocess.run(
            ["ssh-keygen", "-t", "ed25519", "-N", "", "-f", str(KEY_PATH), "-C", "v027-pod"],
            check=True,
        )
...
def pick_gpu_types(api_key: str) -> list[tuple[str, str, float]]:
    ...
    if not out:
        sys.stdout.flush(); print(f"  No exact-match GPU available. Saw {len(by_name)} types:")
        # Best-effort fuzzy: anything with "H100" / "H200" / "A100" in the name.
        for nm, g in by_name.items():
            if any(tok in nm.upper() for tok in ("H100", "H200", "A100")):
                price = (g.get("lowestPrice") or {}).get("uninterruptablePrice")
                if price is not None:
                    sys.stdout.flush(); print(f"    fuzzy-candidate: {nm:30s} ${price:.3f}/hr")
                    out.append((g["id"], nm, price))
        if not out:
...
def fire_pod(api_key: str, pubkey: str, gpu_type_id: str,
    ...
    variables = {
        "input": {
            ...
            "name": f"hammerstein-v027-{int(time.time())}",
            "env": [
...
def main() -> int:
    ...
    try:
        ...
        sys.stdout.flush(); print()
        sys.stdout.flush(); print("  Launching run_v027_pod.sh in tmux session 'v027'...")
        launch = (
            "cd /workspace/hammerstein-model && "
            "tmux new-session -d -s v027 "
            "'bash training/24-7-variant/run_v027_pod.sh > /workspace/v027-run.log 2>&1' && "
            "tmux ls"
        )
        ...
        sys.stdout.flush(); print()
        sys.stdout.flush(); print("  Polling for HF upload sentinel (train + GGUF + HF push ~30-50 min on H100; ~45-75 min on A6000)...")
        sentinel_path = "/workspace/v027-hf-upload-done"
        deadline = time.time() + 120 * 60   # 2hr ceiling — generous for any GPU tier
        last_log_size = -1
        ...
        while time.time() < deadline:
            ...
            try:
                check = (
                    ...
                    f"else wc -c < /workspace/v027-run.log 2>/dev/null || echo 0; fi"
                )
            ...
            if log_size != last_log_size:
                try:
                    rc2, tail = ssh_run(ip, port, "tail -8 /workspace/v027-run.log 2>/dev/null", timeout=30)
        ...
        if not upload_done:
            ...
            try:
                _, tail = ssh_run(ip, port, "tail -120 /workspace/v027-run.log 2>/dev/null", timeout=60)
        ...
        gguf_filename = "hammerstein-7b-v027-q5_k_m.gguf"
        adapter_filename = "lora-adapter-v027.tar.gz"
    ...
    sys.stdout.flush(); print()
    sys.stdout.flush(); print("=== v0.2.7 pod fire complete ===")
    sys.stdout.flush(); print()
    sys.stdout.flush(); print(f"GGUF: {LOCAL_STORE}/hammerstein-7b-v027-q5_k_m.gguf")
    return 0
