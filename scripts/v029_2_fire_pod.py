#!/usr/bin/env python3
r"""v0.2.9.2 pod fire (apply_chat_template fix retrain).

Mirror of v028_fire_pod.py with version-bumped paths.
"""
...
LOCAL_STORE = Path("D:/hammerstein-store/models/v0.2.9.2")

RUNPOD_API = "https://api.runpod.io/graphql"

HF_REPO_ID = "lerugray/hammerstein-7b-v029-2"
...
PREFERRED_GPU_TYPES = [
    "H200 SXM", "H100 NVL", "H100 SXM", "H100 80GB HBM3", "H100 PCIe",
    "A100 SXM4-80GB", "A100 80GB PCIe", "A100 SXM", "A100 80GB",
    "RTX 6000 Ada Generation", "L40S", "L40",
    "RTX A6000", "RTX 4090", "A40", "RTX A5000", "RTX A4500",
]
...
def ensure_ssh_keypair() -> str:
    ...
    if not KEY_PATH.exists():
        ...
        subprocess.run(["ssh-keygen", "-t", "ed25519", "-N", "", "-f", str(KEY_PATH),
                        "-C", "v029-2-pod"], check=True)
...
def fire_pod(api_key: str, pubkey: str, gpu_type_id: str,
    ...
    mutation($input: PodFindAndDeployOnDemandInput!) {
      podFindAndDeployOnDemand(input: $input) { id machineId desiredStatus env }
    }
    """
    variables = {
        "input": {
            "cloudType": "SECURE", "gpuCount": 1, "gpuTypeId": gpu_type_id,
            "containerDiskInGb": 100, "volumeInGb": 0,
            "minMemoryInGb": 16, "minVcpuCount": 4,
            "imageName": template,
            "ports": "22/tcp,8888/http",
            "name": f"hammerstein-v029-2-{int(time.time())}",
...
def ssh_write_file(ip: str, port: int, remote_path: str, content: str, mode: str = "600") -> None:
    cmd = (f"umask 077 && mkdir -p \"$(dirname '{remote_path}')\" "
           f"&& cat > '{remote_path}' && chmod {mode} '{remote_path}'")
    full_cmd = ["ssh", "-i", str(KEY_PATH), "-p", str(port),
                "-o", "StrictHostKeyChecking=no", "-o", "UserKnownHostsFile=/dev/null",
                "-o", "LogLevel=ERROR", "-o", "ConnectTimeout=30", f"root@{ip}", cmd]
...
def main() -> int:
    ...
    sys.stdout.flush(); print("=== v0.2.9.2 pod fire (apply_chat_template fix) ===")
    sys.stdout.flush(); print(f"  Local store target: {LOCAL_STORE}")
    sys.stdout.flush(); print(f"  HF repo target:     {HF_REPO_ID}")
    sys.stdout.flush(); print(f"  Base adapter:       v0.2.6.2 (clean rebase, HF pull pod-side)")
    ...
    try:
        ...
        sys.stdout.flush(); print()
        sys.stdout.flush(); print("  Launching run_v029_2_pod.sh in tmux session 'v029-2'...")
        launch = (
            "cd /workspace/hammerstein-model && "
            "tmux new-session -d -s v029-2 "
            "'bash training/24-7-variant/run_v029_2_pod.sh > /workspace/v029-2-run.log 2>&1' && "
            "tmux ls"
        )
        ...
        sentinel_path = "/workspace/v029-2-hf-upload-done"
        ...
        while time.time() < deadline:
            time.sleep(60)
            try:
                check = (f"if [ -f {sentinel_path} ]; then echo READY; cat {sentinel_path}; "
                         f"else wc -c < /workspace/v029-2-run.log 2>/dev/null || echo 0; fi")
            ...
            if log_size != last_log_size:
                try:
                    rc2, tail = ssh_run(ip, port, "tail -8 /workspace/v029-2-run.log 2>/dev/null", timeout=30)
        ...
        if not upload_done:
            ...
            try:
                _, tail = ssh_run(ip, port, "tail -120 /workspace/v029-2-run.log 2>/dev/null", timeout=60)
        ...
        gguf_filename = "hammerstein-7b-v029-2-q5_k_m.gguf"
        adapter_filename = "lora-adapter-v029-2.tar.gz"

        sys.stdout.flush(); print(f"    pulling {gguf_filename} ...")
        gguf_local = hf_hub_download(repo_id=HF_REPO_ID, filename=gguf_filename,
    ...
    sys.stdout.flush(); print()
    sys.stdout.flush(); print("=== v0.2.9.2 pod fire complete ===")
    sys.stdout.flush(); print(f"GGUF: {LOCAL_STORE}/hammerstein-7b-v029-2-q5_k_m.gguf")
    return 0
