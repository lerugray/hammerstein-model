#!/usr/bin/env python3
"""v0.2.9.2 additions concat + sanitize.

Per docs/handoffs/v0.2.9.2-from-privategs-orchestrator-2026-05-26.md:
continued LoRA from v0.2.9.1 base. Same v0.2.9.1 inclusion list PLUS
4 new files targeting the 3 production-smoke failures:

  - v0.2.9.2-tool-dispatch-natural-language.jsonl (18 pairs, 5x) —
    addresses confabulation on current-info prompts
  - v0.2.9.2-url-read-no-json-tail.jsonl (9 pairs, 8x) — addresses
    JSON-array-tail leak on URL-read prompts
  - v0.2.9.2-retry-shape-reinforcement.jsonl (10 pairs, 5x) —
    addresses tu-03/tu-04 retry-shape eval failures
  - v0.2.9.2-casual-chat-no-meta-leakage.jsonl (8 pairs, 5x) —
    addresses casual-chat → deployment-fact leak
"""
...
ADDITIONS_FILES: list[tuple[str, int]] = [
    # v0.2.9.1 inclusion list (unchanged)
    ...
    ("v0.2.9-tool-routing-alignment-augmented.jsonl",       5),
    ("v0.2.9-tool-use-judgment-augmented.jsonl",            5),
    ...
    ("v0.2.9.1-tool-use-judgment-expanded.jsonl",           5),
    ("v0.2.9.1-high-signal-extraction-reinforcement.jsonl", 5),
    # v0.2.9.2 NEW
    ("v0.2.9.2-tool-dispatch-natural-language.jsonl",       5),
    ("v0.2.9.2-url-read-no-json-tail.jsonl",                8),
    ("v0.2.9.2-retry-shape-reinforcement.jsonl",            5),
    ("v0.2.9.2-casual-chat-no-meta-leakage.jsonl",          5),
]

OUT = DATA_DIR / "ray-stack-sft-v0.2.9.2-additions.jsonl"
...
def main() -> int:
    ...
    with OUT.open("w", encoding="utf-8") as fout:
        for fname, oversample in ADDITIONS_FILES:
            ...
            for obj in rows:
                ...
                # Also strip rejected_anti_example field if present (training-time noise)
                if "rejected_anti_example" in obj:
                    obj.pop("rejected_anti_example", None)
                if "tools" in obj:
    ...
    print(f"v0.2.9.2 additions assembled at {OUT}")
