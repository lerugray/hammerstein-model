#!/usr/bin/env python3
"""v0.2.3 continued-LoRA training — voice fix iteration on top of v0.2.2.

Changes vs train_v022_continued.py:
- v0.2.3 additions = v0.2.2 additions (354) + 90-pair voice anchor (40
  hand-crafted + 50 sonnet-extended in the same register). The voice
  anchor targets the v0.2.2 production failure mode: model defaults to
  formal project-doc voice on casual relational prompts (Ray's exact
  complaint after the system-prompt fix landed).
- Voice direction (locked via Ray's AskUserQuestion picks):
  - Hybrid casual register, Hammerstein character preserved
  - Relational reciprocation allowed (and trained) on relational prompts
  - Audit / knowledge / refusal prompts still hold the strict
    no-engagement-pushing rule
- The system-prompt fact-injection (deployed 2026-05-23 in
  scripts/rung1_server.py) already lifted self-state honesty from 55.6%
  to 80.0% manual-graded. v0.2.3 SFT addresses what the system prompt
  can't: the trained voice register itself.

Kept from v0.2.2 (proven good):
- LR 2e-4
- 2 epochs
- 3x oversample on the v0.2.3 additions
- v3a 250-pair English anchor
- v0.1 Ray-stack full
- All v0.2.1 audit-discrimination + fabrication-discrimination +
  short-prompt + restyled + v0.2.2 tool-call carryover

Training mix:
- data/ray-stack-sft-v0.2.3-additions.jsonl (444 pairs, oversampled 3x
  = 1,332)
- data/ray-stack-sft-v0.1-combined.jsonl (541 pairs, full)
- tools/distill/data/synthetic-v3a-2026-05-09.jsonl (sample 250)

Total mix ~2,123 pairs. Estimated A5000 ~$0.60 / ~30 min train.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--v023-additions",
                   default="data/ray-stack-sft-v0.2.3-additions.jsonl")
    ...
    p.add_argument("--output",
                   default="training/24-7-variant/output/qwen7b-v023-continued")
...
def build_mix(args: argparse.Namespace) -> tuple[list[str], dict]:
    ...
    v023_path = Path(args.v023_additions)
    if v023_path.exists():
        v023_rows = load_jsonl(args.v023_additions)
        for _ in range(MIX_TARGETS["v023_additions_oversample"]):
            for row in v023_rows:
                text = format_example(row)
                if text:
                    examples.append(text)
        counts["v023_additions"] = len(v023_rows) * MIX_TARGETS["v023_additions_oversample"]
    else:
        print(f"  WARN: v0.2.3 additions missing: {args.v023_additions}")
        counts["v023_additions"] = 0
...
def run_training(args: argparse.Namespace) -> None:
    ...
    print("\nStarting continued training (v0.2.3)...")
...
def main() -> None:
    args = parse_args()
    print("=== Qwen2.5-7B v0.2.3 continued-LoRA SFT ===")
    print(f"  Base model:       {BASE_MODEL}")
    print(f"  Starting adapter: {args.v3a_adapter}")
    print(f"  v0.2.3 additions: {args.v023_additions} (x{MIX_TARGETS['v023_additions_oversample']} oversample)")
    ...
    if not Path(args.v023_additions).exists():
        print(f"ERROR: v0.2.3 additions not found at {args.v023_additions}")
        print(f"  Run: python scripts/v023_concat_sanitize.py")
        sys.exit(1)
